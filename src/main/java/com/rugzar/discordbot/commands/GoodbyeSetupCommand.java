...
public class GoodbyeSetupCommand extends ListenerAdapter {

    @Override
    public void onSlashCommandInteraction(SlashCommandInteractionEvent event) {

        if (!event.getName().equals("guleguleayarla")) return;
        ...
        TextChannel channel = event.getOption("kanal")
                .getAsChannel()
                .asTextChannel();

        String sql = """
                INSERT INTO guild_settings (guild_id, goodbye_channel_id)
                VALUES (?, ?)
        ...
        try (Connection connection = Database.getConnection();
            ...
            event.reply("✅ Güle güle kanalı **" + channel.getAsMention() + "** olarak ayarlandı.")
                    .queue();

        } catch (Exception e) {
            e.printStackTrace();

            event.reply("❌ Güle güle kanalı kaydedilirken bir veritabanı hatası oluştu.")
                    .setEphemeral(true)
                    .queue();
        }
    }
