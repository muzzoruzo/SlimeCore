...
public class WelcomeSetupCommand extends ListenerAdapter {

    @Override
    public void onSlashCommandInteraction(SlashCommandInteractionEvent event) {

        if (!event.getName().equals("hosgeldinayarla")) return;
        ...
        try (Connection connection = Database.getConnection();
            ...
            event.reply("✅ Hoş geldin kanalı **" + channel.getAsMention() + "** olarak ayarlandı.")
                    .queue();

        } catch (Exception e) {
            e.printStackTrace();

            event.reply("❌ Hoş geldin kanalı kaydedilirken bir veritabanı hatası oluştu.")
                    .setEphemeral(true)
                    .queue();
        }
    }
